...
export default function Toast({ message, type = 'success', onDone }: ToastProps) {
    ...
    return (
        <div
            className={`fixed left-1/2 -translate-x-1/2 flex items-center gap-3 px-6 py-4 rounded-xl shadow-2xl z-[100] transition-all animate-in fade-in slide-in-from-top-4 ${type === 'success' ? 'bg-slate-900 text-white' : 'bg-red-500 text-white'}`}
