...
export default function ExerciseChart({ chartData, friendName, comparisonFriendId }: Props) {
    ...
    return (
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex-1">
            ...
            >
                <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest flex items-center gap-2 transition-colors group-hover:text-slate-600">
                ...
                <div className="text-slate-400 p-1 bg-slate-50 rounded transition-colors group-hover:bg-slate-100 group-hover:text-slate-600">
                    {isExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                </div>
            </button>

            {isExpanded && (
                <div className="h-48 w-full mt-4 animate-fade">
